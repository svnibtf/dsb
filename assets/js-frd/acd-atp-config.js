acd_atp_origem = ['NENHUMA', 'VESTIBULAR', 'ENEM', 'DESAFIOS-SESI', 'DESAFIOS-PV10', 'CONCURSOS', 'FUVEST', 'VUNESP', 'UNICAMP', 'UNIFESP', 'UFPR', 'UEL', 'UEM', 'UFPG', 'CESGRANRIO', 'ITA', 'IME', 'AFA', 'UEMG', 'OUTRAS FEDERAIS', 'OUTRAS ESTADUAIS', 'ESTUDOS-PV10', 'MACKENZIE', 'FAAP', 'POLI', 'FEI', 'METODISTAS', 'PUCs', 'FCC - CARLOS CHAGAS', 'OUTROS'];
for(var index in acd_atp_origem){
	$('#acd_atp_origem, #pes_acd_atp_origem').append('<option value="'+acd_atp_origem[index]+'">'+acd_atp_origem[index]+'</option>');	
}
$('#acd_atp_ano').append('<option value="INDEFINIDO">INDEFINIDO</option>');	
for(var i= 2016; i>1960; i--){
	$('#acd_atp_ano, #pes_acd_atp_ano').append('<option value="'+i+'">'+i+'</option>');	
}
acd_atp_alter_corr = ['A','B', 'C', 'D', 'E'];
for(var index in acd_atp_alter_corr){
	var valor = acd_atp_alter_corr[index].toUpperCase();
	$('#acd_atp_alter_corr, #pes_acd_atp_alter_corr').append('<option value="'+valor+'">'+valor+'</option>');	
}
